 ...
 * Redirect
 *
 * Redirect .301 file (if it exists).
 *
 */
exports.redirect = function(req, res, next) {
  ...
  if(ext == '301') {
    ...
    var pathn = path.join(req.setup.publicPath, parse(req).pathname) + '.301';

    if(fs.existsSync(pathn)) {
      var redirectLocation = fs.readFileSync(pathn);
      res.statusCode = 301;
      res.setHeader('Location', redirectLocation);
      res.end('Redirecting to ' + utils.escape(redirectLocation) + '/');
    }else{
      next();
    }
  }
