...
/**
 * Templates root (error, 404, index pages shipped with harp)
 */
var templates = terraform.root(__dirname + "/templates")

/**
 * Respond
 *
 * Writes an HTTP response with the correct Content-Type (incl. charset)
 * and Content-Length headers. `type` defaults to html.
 */
var respond = function(rsp, body, statusCode, type){
  type        = type || helpers.mimeType("html")
  var charset = mime.charsets.lookup(type)
  rsp.setHeader('Content-Type', type + (charset ? '; charset=' + charset.toLowerCase() : ''))
...
var measureBody = function(contentLength){
  ...
  return `(${ helpers.toHuman(contentLength) })`
}

exports.setupLog = function(req, rsp, next){
  onFinished(rsp, function (err, rsp) {
    var contentLength = parseInt(rsp.getHeader('content-length')) || null
...
exports.notMultihostURL = function(req, rsp, next){
  ...
  if(hostname == "127.0.0.1" || hostname == "localhost"){
    ...
    var body = "Local server does not support history. Perhaps you are looking for <a href='" + link + "'>" + link + "</a>."
    rsp.statusCode = 307
    rsp.end(body)
  }else if(arr.length > 4){
    arr.shift()
  ...
  return function(req, rsp, next){
    ...
    if(arr.length == 2){
      fs.readdir(dirPath, function(err, files){
        ...
        templates.render("index.jade", { pkg: pkg, projects: projects, layout: "_layout.jade" }, function(error, body){
          rsp.end(body)
        });
      })
    } else {
...
exports.fallback2 = function(req, rsp, next){
...
exports.fallback = function(req, rsp, next){
...
var custom200static = function(req, rsp, next){
  fs.readFile(path.resolve(req.setup.publicPath, "200.html"), function(err, contents){
    if(contents){
      respond(rsp, contents.toString(), 200)
    }else{
      next()
    }
  })
...
var custom404static = function(req, rsp, next){
  fs.readFile(path.resolve(req.setup.publicPath, "404.html"), function(err, contents){
    if(contents){
      respond(rsp, contents.toString(), 404)
    }else{
      next()
    }
  })
...
var default404 = function(req, rsp, next){
  ...
  templates.render("404.jade", locals, function(err, body){
    respond(rsp, body, 404)
  })
}
...
exports.setup = function(req, rsp, next){
  ...
  try{
    ...
    return templates.render("error.jade", locals, function(err, body){
      rsp.statusCode = 500
      rsp.end(body)
    })
  }
    ...
    req.poly.render(sourceFile, function(error, body){
      if(error){
        ...
        if(terraform.helpers.outputType(sourceFile) == 'css'){
          var mimeType = helpers.mimeType(terraform.helpers.outputType(sourceFile))
          respond(rsp, helpers.cssError(locals), 200, mimeType)
        }else{
          templates.render("error.jade", locals, function(err, body){
            respond(rsp, body, 500)
          })
        }
      }else{
        // 404
