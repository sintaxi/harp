...
exports.parseHarpConfig = function(req, rsp, next){
  ...
  if(!contents){
    ...
    try{
      req.config = JSON.parse(contents)
    }catch(err){
      // TODO: make this better
      rsp.statusCode = 500
      rsp.end("Invalid harp.json")
    }
  }

  if(!req.config.hasOwnProperty('globals')){
    req.config['globals'] = {}
  }

  req.config.globals.environment = process.env.NODE_ENV || "development"
...
exports.notFound = function(req, rsp){
  //var poly = polymer.helpers.findFirstFile(path.joinreq.projectPath, "public")
