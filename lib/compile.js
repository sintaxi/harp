...
module.exports = function(projectPath, outputPath, callback){
  ...
  helpers.ls(publicPath, function(err, results){
    ...
    var less = function(callback){
      // write jade
      async.forEach(results, function(f, cb){
        parsers.less(path.resolve(publicPath, f), function(err, html){
          if(err){
            if(err !== "NoMatch"){
              cb(err)
            }else{
            ...
            var outputFilePath = path.resolve(outputPath, f).replace(".less", ".css")
            var outputDirPath  = path.dirname(outputFilePath)
            mkdirp(outputDirPath, function(){
              fs.writeFile(outputFilePath, html, function(err){
                cb(null)
    ...
    var remainingAssets = function(err, callback){
    ...
    async.parallel([jade, less], remainingAssets)
