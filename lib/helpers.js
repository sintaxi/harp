...
exports.prime = function(primePath, options, callback){
  ...
  fse.mkdirp(primePath, function(){
    fse.readdir(primePath, function(error, contents){
      ...
      // console.log("IN CONTENTS", filePath);
      ...
      var cnameRegExp = new RegExp(/^CNAME/);
      var readmeRegExp = new RegExp(/^readme.md/i);
      ...
      contents.forEach(function(i){
         ...
         */
        if(filePath === ignorePath || gitRegExp.test(i) || cnameRegExp.test(i) || readmeRegExp.test(i)){
          count++
