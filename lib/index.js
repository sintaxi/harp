...
exports.server = function(dirPath, options, callback){
  ...
  // get browserify plugins...
  // try{
  //   var pkg  = require(setup.projectPath + "/package.json") || {}
  //   var opts = pkg["browserify"] || {}
  // }catch(e){}

  // if (!opts){
  //   try{
  //     var pkg  = require(setup.cwd + "/package.json") || {}
  //     var opts = pkg["browserify"] || {}
  //   }catch(e){
  //     var opts = {}  
  //   }
  ...
  //   cache: {},
  //   packageCache: {},
  //   plugin: [watchify].concat(plugins)
  // })
  //console.log("Browserify:", JSON.stringify(opts, null, 2))
  //var b = browserify(setup.publicPath + "/bundle.cjs", opts)

  // console.log(__dirname)

  var b = browserify({
    basedir: path.resolve(__dirname + "/.."),
    entries: [setup.publicPath + "/bundle.jsx"],
    cache: {},
    packageCache: {},
  ...
  var update = function(info){
    ...
    stream.on("error", function(error){
      cjs["bundle.js"] = 'console.log("'+ error.toString() +'")'
    })
      ...
      terra.render(file, function(error, body){
        if(error){
          ...
          if(body){
            ...
            if (file === "bundle.jsx"){
              console.log("writting bundle.jsx")
              // get browserify plugins...
              try{
                var pkg = require(setup.projectPath + "/package.json")["browserify"]
                var plugins = []
              }catch(e){
                var plugins = []
              }
              ...
              var b = browserify({
                basedir: path.resolve(__dirname + "/.."),
                entries: [path.resolve(setup.publicPath, "bundle.jsx")],
                transform: [["babelify", { "presets": ["@babel/preset-env", "@babel/preset-react"] }]],
                plugin: []
              })
